...
def get_or_create_conversation(
    conn, phone: str, display_name: str = "", *, revive: bool = False
) -> dict | None:
    ...
    if revive:
        clear_conversation_tombstone(conn, phone_e164)
    elif conversation_is_tombstoned(conn, phone_e164):
        return None
    row = conn.execute(
...
def conversation_is_tombstoned(conn, phone: str) -> bool:
    phone_e164 = wa.normalise_whatsapp_number(phone)
    if not phone_e164:
        return False
    ensure_communication_hub_schema(conn)
    row = conn.execute(
        "SELECT 1 FROM wa_conversation_tombstones WHERE phone_e164 = ? LIMIT 1",
        (phone_e164,),
    ).fetchone()
    return bool(row)


def clear_conversation_tombstone(conn, phone: str) -> None:
    phone_e164 = wa.normalise_whatsapp_number(phone)
    if not phone_e164:
        return
    ensure_communication_hub_schema(conn)
    conn.execute(
        "DELETE FROM wa_conversation_tombstones WHERE phone_e164 = ?",
        (phone_e164,),
    )
...
def delete_conversation(conn, conversation_id: int) -> bool:
    """Remove a hub conversation and its local message history."""
    ensure_communication_hub_schema(conn)
    ...
    if phone:
        conn.execute(
            """INSERT INTO wa_conversation_tombstones (phone_e164, deleted_at)
               VALUES (?, datetime('now','localtime'))
               ON CONFLICT(phone_e164) DO UPDATE SET
                 deleted_at = excluded.deleted_at""",
            (phone,),
        )
    conn.execute("DELETE FROM wa_messages WHERE conversation_id = ?", (cid,))
    conn.execute("DELETE FROM wa_conversations WHERE id = ?", (cid,))
    return True
...
def send_conversation_document_bytes(
    conn,
    conversation_id: int,
    *,
    data: bytes,
    ...
    """Upload generated bytes to WhatsApp and send as a document on the conversation."""
    import os
    import tempfile
    ...
    safe_name = os.path.basename(str(filename or "document.pdf")).strip() or "document.pdf"
    mime_type = (mime or "application/pdf").strip().lower() or "application/pdf"
    media_type = "document"
    if mime_type in _ALLOWED_IMAGE_TYPES or mime_type.startswith("image/"):
        media_type = "image"
    ...
    try:
        ...
        ok_up, err_up, body_up = wa.upload_media_file(tmp_path, mime_type)
        media_id = ""
        ...
        ok, err, payload = wa.send_media_message(
            ...
            filename=safe_name if media_type == "document" else "",
            caption=caption,
        )
        wa_message_id = wa.first_message_id(payload) if ok else ""
        body_preview = (caption or "").strip() or safe_name
        if ok:
            message = append_message(
                conn,
                conversation_id,
...
def send_conversation_attachment(
    ...
    *,
    ...
    return send_conversation_document_bytes(
        conn,
        conversation_id,
        data=data,
        filename=filename,
...
def register_communication_hub(app, *, pop_auth_notice, get_user):
    ...
    def communication_hub_api_conversation_create():
        ...
        try:
            conversation = get_or_create_conversation(
                conn, phone_e164, display_name=display_name, revive=True
            )
            conn.commit()
        finally:
            conn.close()
    ...
    @app.route("/communication-hub/api/conversations/<int:conversation_id>", methods=["DELETE"])
    def communication_hub_api_conversation_delete(conversation_id):
        conn = get_db()
        try:
            ...
            conn.close()
        return jsonify({"ok": True, "id": int(conversation_id)})
