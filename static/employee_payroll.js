...
(function () {
  ...
  window.submitPayrollLock = function submitPayrollLock(year, month, label) {
    if (!window.confirm(
      'Lock ' + label + '? After locking, NO edits are allowed for this month — attendance, credits, repayments, tip incentive, and wage fields — including for administrators. This cannot be unlocked.'
    )) return;
    var cfg = pageConfig();
    var form = document.createElement('form');
    form.method = 'POST';
