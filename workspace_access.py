...
def user_can_access_unit_master(user):
    """Unit Master via Master hub or Stores Product Master."""
    if not user:
        return False
    if user.get("is_admin"):
        return True
    if user_can_access_master_submodule(user, "unit"):
        return True
...
def mobile_module_access(user):
    ...
    return {
        ...
        "pos": bool(pos_invoice or pos_tables),
        "kot": bool(pos_invoice or pos_tables),
        "pos_bar": bool(bar_invoice or bar_tables),
