...
/**
 * Silent self-update via PackageInstaller Session API for the WebView shell APK.
 * Same package + same signing cert required.
 */
public final class SilentUpdateHelper extends BroadcastReceiver {
    public static final String ACTION_INSTALL_STATUS =
            "com.hotelbellelite.hbe.INSTALL_STATUS";
    public static final String EXPECTED_PACKAGE = "com.hotelbellelite.hbe";
    private static final String TAG = "HbeShellUpdate";

    public interface StatusCallback {
        void onInstallBroadcast(String state, String message);
    }

    private static volatile StatusCallback sStatusCallback;

    public static void setStatusCallback(StatusCallback callback) {
        sStatusCallback = callback;
    }

    private static void fireStatus(String state, String message) {
        StatusCallback cb = sStatusCallback;
        if (cb == null) {
            return;
        }
        try {
            cb.onInstallBroadcast(state, message == null ? "" : message);
        } catch (Exception e) {
            Log.w(TAG, "status callback failed", e);
        }
    }

    private static final String SESSION_NAME = "hbe-shell-ota";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (status == PackageInstaller.STATUS_PENDING_USER_ACTION) {
            ...
            fireStatus("pending_user_action", message);
        } else if (status == PackageInstaller.STATUS_SUCCESS) {
            fireStatus("installing", message);
            relaunch(app);
        } else {
            Log.w(TAG, "install not successful: " + status);
            fireStatus("error", message != null ? message : ("status=" + status));
        }
    }
