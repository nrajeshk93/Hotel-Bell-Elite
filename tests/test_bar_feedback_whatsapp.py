"""Bar Generate Invoice feedback WhatsApp (Meta template bar_feedback)."""
...
class BarFeedbackWhatsAppUnitTests(unittest.TestCase):
    def test_template_name_default(self):
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("WHATSAPP_BAR_FEEDBACK_TEMPLATE", None)
            os.environ.pop("WHATSAPP_BAR_FEEDBACK_TEMPLATE_LANGUAGE", None)
            name, lang = bfw.bar_feedback_template_config()
        self.assertEqual(name, "bar_feedback_link")
        self.assertEqual(lang, "en")

    def test_header_image_file_exists(self):
        path = bfw.bar_feedback_header_image_path()
        self.assertTrue(os.path.isfile(path), path)


class BarFeedbackWhatsAppSendTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_dry_run_mints_24h_invite_and_payload(self):
        with mock.patch.dict(os.environ, {"WHATSAPP_DRY_RUN": "1"}, clear=False):
            result = bfw.send_bar_feedback_whatsapp(
                {
                    "customer_name": "Rajesh",
                    "mobile": "9876543210",
                    "mobile_country": "+91",
                    "outlet": "bar",
                    "order_no": "IBH/25-26/1",
                }
            )
        ...
        self.assertEqual(invite.get("source"), "bar")
        self.assertEqual(invite.get("expires_in_hours"), 24)
        self.assertEqual(result.get("template_name"), "bar_feedback_link")
...
class BarFeedbackWhatsAppApiTests(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
        self.tmp.close()
        self.db_path = self.tmp.name
    ...
    def test_endpoint_maps_to_point_of_sale_bar_invoice(self):
        self.assertEqual(
            get_endpoint_dashboard_module(
    ...
    def test_api_dry_run_ok(self):
        ...
        self.assertEqual(body.get("template_name"), "bar_feedback_link")
        self.assertIn("/f/", body.get("share_url") or "")
        self.assertEqual((body.get("invite") or {}).get("source"), "bar")
