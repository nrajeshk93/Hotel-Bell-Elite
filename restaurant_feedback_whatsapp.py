"""Send restaurant (Spices) feedback invite via WhatsApp Meta template ``spices_feedback_template``.

Meta template (approved): ``spices_feedback_template``
  HEADER: IMAGE (HD Spices feedback art)
  BODY: one variable — guest display name
  BUTTON: URL CTA ``Share Your Feedback`` → ``https://belleliteaccounts.com/f/{{1}}``

Env (optional overrides):
  WHATSAPP_SPICES_FEEDBACK_TEMPLATE=spices_feedback_link
...
DEFAULT_TEMPLATE_NAME = "spices_feedback_link"
DEFAULT_TEMPLATE_LANGUAGE = "en"
HEADER_IMAGE_REL = os.path.join("static", "spices_feedback_whatsapp_header.png")


def spices_feedback_template_config() -> tuple[str, str]:
    name = (
        os.environ.get("WHATSAPP_SPICES_FEEDBACK_TEMPLATE") or DEFAULT_TEMPLATE_NAME
    ).strip()
    lang = (
        os.environ.get("WHATSAPP_SPICES_FEEDBACK_TEMPLATE_LANGUAGE")
        or DEFAULT_TEMPLATE_LANGUAGE
    ).strip()
    return name or DEFAULT_TEMPLATE_NAME, lang or DEFAULT_TEMPLATE_LANGUAGE


def spices_feedback_header_image_path() -> str:
    """Absolute path to the HD PNG uploaded as the template IMAGE header."""
    here = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(here, HEADER_IMAGE_REL)


def send_restaurant_feedback_whatsapp(
    bill_snapshot: dict[str, Any],
    *,
    user_id=None,
    note: str = "",
) -> dict[str, Any]:
    """Mint a 24h restaurant feedback invite and send ``spices_feedback_template``.
    ...
    template_name, template_lang = spices_feedback_template_config()
    outlet = str(data.get("outlet") or "restaurant").strip() or "restaurant"
    ...
    if not note_clean:
        bits = ["restaurant generate-invoice feedback"]
        if order_no:
    ...
    image_path = spices_feedback_header_image_path()
    if not os.path.isfile(image_path):
        return {
            "ok": False,
            "error": "Spices feedback WhatsApp header image is missing on the server.",
            "status": 500,
